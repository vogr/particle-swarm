...
void step4_base(struct pso_data_constant_inertia *pso)
{
  ...
  for (int i = 1; i < pso->population_size; i++)  // unroll with mult. (4?) accumulators
  {
...
void step4_opt1(struct pso_data_constant_inertia *pso)
{
  ...
  for (int i = 0; i < pop_size; i++)
  {
    ...
    for (; k < dim - 3; k += 4)
    {
      ...
      pso_y_i_dim[k] = pso_x_i_dim[k]; // PSO_Y(pso, i)[k] = PSO_X(pso, 0, i)[k];
      pso_y_i_dim[k1] = pso_x_i_dim[k1];
      pso_y_i_dim[k2] = pso_x_i_dim[k2];
      pso_y_i_dim[k3] = pso_x_i_dim[k3];
    }
    for (; k < dim; k++)
    {
