...
static const char *fixed_steps_descriptions[3] = {
    "Evaluate points of space-filling design AND Determine initial swarm "
    "positions",
...
static void alloc_timer(size_t time_max, size_t fixed_steps_count_,
                        size_t repeated_steps_count_)
{
  fixed_steps_cycles = malloc(fixed_steps_count_ * sizeof(double));
  repeated_steps_cycles =
      malloc(time_max * repeated_steps_count_ * sizeof(double));
...
static void timer_print_statistics(size_t time_max)
{
  ...
  for (size_t step = 0; step < fixed_steps_count; ++step)
  {
    printf("Step %d: %lf cycles (%lf%%)\n", step + 1, FIXED_STEP(step),
           100 * FIXED_STEP(step) / total);
  ...
  for (size_t step = 0; step < repeated_steps_count; ++step)
  {
    ...
    printf("Step %d: %lf cycles (%lf%%), %lf cycles per step on average\n",
           step + 5, step_total, 100 * step_total / total,
           step_total / time_max);
