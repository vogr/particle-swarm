...
static void pack_4(double *dst, double *src, int LDA, int M, int N)
{
  for (int j = 0; j < N; ++j)
  {
    for (int i = 0; i < M; ++i)
...
void dgemm_4(int M, int N, int K, double alpha, double *restrict A, int LDA,
             double *restrict B, int LDB, double beta, double *restrict C,
             int LDC)
{
  double *AL = scratch_a;
  ...
  for (j = 0; j < N; j += d_j)
  {
    ...
    {
      d_k = MIN(K - k, K_BLOCK);
      pack_4(BL, &TIX(B, LDB, k, j), LDB, d_k, d_j);
      for (i = 0; i < M; i += d_i)
      {
        d_i = MIN(M - i, M_BLOCK);
        pack_4(AL, &TIX(A, LDA, i, k), LDA, d_i, d_k);
        dgemm_1T(d_i, d_j, d_k, alpha, //
                 AL, d_i,              //
                 BL, d_k,              //
                 beta, &TIX(C, LDC, i, j), LDC);
...
// -------------
// DGEMM 5 helpers
// XXX learn your lession from dgemm_2 and make
// macros for cleaner inner loops.
...
#define DO_8x4B_5                                                              \
  ...
  b_k0_j2 = _mm256_set1_pd(*(bbuff + 2));                                      \
  b_k0_j3 = _mm256_set1_pd(*(bbuff + 3));                                      \
...
#define INIT_4x1B_5 c_i0_j0 = _mm256_setzero_pd();
...
#define DO_2x4B_5                                                              \
  ...
  db_k0_j2 = _mm_set1_pd(*(bbuff + 2));                                        \
  db_k0_j3 = _mm_set1_pd(*(bbuff + 3));                                        \
  dc_i0_j2 = _mm_fmadd_pd(da_i0_k0, db_k0_j2, dc_i0_j2);                       \
  dc_i0_j3 = _mm_fmadd_pd(da_i0_k0, db_k0_j3, dc_i0_j3);                       \
  ++k, abuff += 2, bbuff += 4;
...
#define INIT_1x4B_5 s_c0 = s_c1 = s_c2 = s_c3 = 0.;
...
#define INIT_1x2B_5 s_c0 = s_c1 = 0.;
...
#define STORE_1x1B_5 TIX(C, LDC, i + 0, j + 0) += s_c0;

#define MICRO_1x1_MMM_5                                                        \
  abuff = A + i * K, bbuff = B + j * K;                                        \
  INIT_1x1B_5;                                                                 \
  for (k = 0; 0 && k < K4_MOD;)                                                \
  {                                                                            \
...
// packing A with 8,4,2,1 blocks
static void pack_a_5(double *dst, double *src, int LDA, int M, int N)
{
  int i, j;
  double *s0, *d0 = dst;
  i = 0;
...
// packing B in 4,2,1 blocks
static void pack_b_5(double *dst, double *src, int LDA, int M, int N)
{
  int i, j;
  double   //
      *s0, //
             ...
             int LDC)
{
  ...
  for (j = 0; j < N; j += d_j)
  {
    ...
    {
      d_k = MIN(K - k, K_BLOCK);
      pack_b_5(BL, &TIX(B, LDB, k, j), LDB, d_k, d_j);
      for (i = 0; i < M; i += d_i)
      {
        d_i = MIN(M - i, M_BLOCK);
        pack_a_5(AL, &TIX(A, LDA, i, k), LDA, d_i, d_k);
...
#define INIT_4x1B_6 c_i0_j0 = _mm256_setzero_pd();
...
#define INIT_1x4B_6 s_c0 = s_c1 = s_c2 = s_c3 = 0.;
...
#define INIT_1x2B_6 s_c0 = s_c1 = 0.;
...
#define STORE_1x1B_6 TIX(C, LDC, i + 0, j + 0) += s_c0;

#define MICRO_1x1_MMM_6                                                        \
  abuff = A + i * K, bbuff = B + j * K;                                        \
  INIT_1x1B_6;                                                                 \
  for (k = 0; 0 && k < K4_MOD;)                                                \
  {                                                                            \
...
// packing A with 8,4,2,1 blocks
static void pack_a_6(double *dst, double *src, int LDA, int M, int N)
{
  int i, j;
  double *s0, *d0 = dst;
  i = 0;
...
// packing B in 4,2,1 blocks
static void pack_b_6(double *dst, double *src, int LDA, int M, int N)
{
  int i, j;
  double   //
      *s0, //
...
// DGEMM 6 attempts to get things to run on virtual registers.
// XXX so far nothing has been obiously successful.
             ...
             int LDC)
{
  ...
  for (j = 0; j < N; j += d_j)
  {
    ...
    {
      d_k = MIN(K - k, K_BLOCK);
      pack_b_6(BL, &TIX(B, LDB, k, j), LDB, d_k, d_j);
      for (i = 0; i < M; i += d_i)
      {
        d_i = MIN(M - i, M_BLOCK);
        pack_a_6(AL, &TIX(A, LDA, i, k), LDA, d_i, d_k);
