                             ...
                             double const *const x, int add)
{
  ...
  for (uint64_t off = 0; off < max_off; off++)
  {
    ...
    if (bloom_check(&bloom->bloom, (char *)neighbor_dual_bin_id,
                    dims * sizeof(uint64_t)))
    {
      ret = 1;
#if DEBUG_BLOOM
...
int rounding_bloom_check(struct rounding_bloom *bloom, int dims,
                         double *const x)
{
